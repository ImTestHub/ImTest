...
try {
    ...
    $data = @{
        'im_test.versionName' = $versionName
        'im_test.versionCode' = $versionCode
        'im_test.commitHash' = $commitHash
        'im_test.buildTime' = $buildTime
    }

    $data | ConvertTo-Json -Compress | Out-File 'im_test_release.json' -Encoding UTF8
